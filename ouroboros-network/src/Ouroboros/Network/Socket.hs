...
import qualified Protocol.Channel as C
import           Protocol.Codec
import qualified Protocol.Core as C
...
demo chain0 updates = do
    ...
    let producerChannel = multiplexCtxDuplex producerCtxs (ChainHeaderSync Responder)
        consumerChannel = multiplexCtxDuplex consumerCtxs (ChainHeaderSync Initiator)

        producerPeer :: C.Peer ChainSyncProtocol (ChainSyncMessage block (Point block))
                             (C.Awaiting StIdle) (C.Finished StDone)
                             IO ()
        producerPeer = chainSyncServerPeer (chainSyncServerExample () producerVar)

        consumerPeer :: C.Peer ChainSyncProtocol (ChainSyncMessage block (Point block))
                             (C.Yielding StIdle) (C.Finished StDone)
                             IO ()
...
multiplexCtxDuplex :: MultiplexCtx -> Conversation -> C.Duplex IO IO CBOR.Encoding BS.ByteString
multiplexCtxDuplex mx conv =
    case M.lookup conv (convs mx) of
         Nothing  -> error $ "Missing conversation " ++ (show conv) -- XXX
         Just ctx -> C.uniformDuplex (write ctx) (read ctx)
  where
    read ctx = do
        chunk <- atomically $ readTBQueue (ccInQ ctx)
        if BS.null chunk
    ...
    write ctx = \encoding -> do
        atomically $ writeTBQueue (ccOutQ ctx) (CBOR.toStrictByteString encoding)
        signalQSem $ sem mx
